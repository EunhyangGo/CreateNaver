 ...
 * 		생성된 데이터 = service로부터 return받은 데이터
 * 		rendering : *.jsp + vo의 결합
 * 4. HTML코드로 생성한 후 web browser로 response 송신.
 * 
 * >> 사소한 문자열 1개를 보낼려고 해도 반드시 
 *  *.jsp파일이 있어야 하고, Rendering 과정을 거쳐야 한다.
 *  
 *  	그래서 Spring 3.5 이후에서는 @Responsebody라는 Annotation을 도입해서
 *  	단순 문자열을 Rendering하지 않고 browser로 보낼 수 있게 되었다.
 *  
 */
@Slf4j
@Controller
...
//		return "home";
//		return "Welcom to Korea" 웰컴 투 코리아라는 걸 보여라 ;
